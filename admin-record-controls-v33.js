(() => {
  ...
  async function changeServiceDate(day, recordId, button) {
    ...
    try {
      if (currentCase?.id) {
        const { data, error } = await sb
          .from('daily_records')
          .select('id,service_day,service_date')
      ...
      console.error('날짜 중복 확인 오류', err);
      return alertMsg(`날짜를 확인하지 못했습니다. ${err?.message || ''}`);
    }

    const message = conflict
      ? `${day}일차 서비스 날짜를 수정하시겠습니까?\n\n` +
        `⚠️ ${newDate} 날짜가 현재 ${conflict.service_day}일차에 입력되어 있습니다.\n` +
        `날짜 중복을 막기 위해 두 일차의 날짜를 서로 바꿉니다.\n\n` +
        `${day}일차: ${oldDate || '미입력'} → ${newDate}\n` +
        `${conflict.service_day}일차: ${newDate} → ${oldDate || '미입력'}\n\n` +
    ...
    try {
      ...
      alertMsg(
        conflict
          ? `날짜를 수정했습니다. ${conflict.service_day}일차와 날짜를 서로 바꿨으며 기존 기록과 서명은 그대로 유지됩니다.`
          : '서비스 날짜를 수정했습니다. 기존 기록과 서명은 그대로 유지됩니다.'
      );
      if (currentCase?.id) {
  ...
  function addAdminDateCorrection(day) {
    ...
    box.innerHTML = `
      ...
      <div class="muted" style="margin-bottom:8px">관리사가 날짜를 잘못 저장했을 때 날짜만 바로잡습니다. 다른 일차와 날짜가 겹치면 두 일차 날짜를 서로 바꿉니다. 기존 기록과 산모 서명은 유지됩니다.</div>
